import { tokTypes, lineBreak } from 'acorn';
...
PeekToken.prototype.isContextual = function isContextual (name) {
  return this.type === tokTypes.name && this.value === name && !this.containsEsc
};
...
function isBindingIdentifier(token, isFor) {
  return token.type === tokTypes.name &&
...
function matchUsingTail(parser, usingEnd, isFor) {
  ...
  if (tail.type === tokTypes.eq || isFor && tail.isContextual("of")) { return MATCH }
  return NO_MATCH
}
...
pp.classifyUsingStart = function() {
  if (this.type !== tokTypes.name || this.containsEsc) { return NO_MATCH }
  if (this.value === "using") {
...
pp.parseForStatementTail = function(node, awaitAt) {
  ...
  if (this.type === tokTypes._var || this.type === tokTypes._const || isLet) {
    ...
    if ((this.type === tokTypes._in || (this.options.ecmaVersion >= 6 && this.isContextual("of"))) && init$1.declarations.length === 1) {
  ...
  if (this.type === tokTypes._in || (isForOf = this.options.ecmaVersion >= 6 && this.isContextual("of"))) {
    if (this.options.ecmaVersion >= 9) {
      if (this.type === tokTypes._in) {
        if (awaitAt > -1) { this.unexpected(awaitAt); }
      } else { node.await = awaitAt > -1; }
